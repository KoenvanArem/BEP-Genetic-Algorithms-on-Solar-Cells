function tc = ObjFun_Final_first(x)
    ...
    % check whether the solar cell has a pyramid structure
    if x(10)==1
    ...
    %=== FUNCTION CALL ===
    % activate option to store scatter matrices
    S.SM = 1;
    
    % activate parallel computing within GenPro4 to speed up the simulations.
    S.parll = 1;
        
    % call GENPRO4
    [Lay,Int] = GENPRO4(Lay,Int,S);
    
    % Save Int(2) since it contains the scatter matrices of the second interface 
    temp = Int(2);
    if x(10)==0
        save('storage_no_pyramids.mat','temp')
    else
        save('storage_pyramids.mat','temp')
    end
